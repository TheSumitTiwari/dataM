...
showbtn.addEventListener("click", function(){
  ...
  document.getElementById('submitbtn').disabled = false
    ...
    localADB.createIndex({
        index: {
            fields: ['aadhaar','class','section','date']
          }
    }).then(function(index) {	
        localADB.find({
            selector: {
            ...
            console.log(result.docs.map(function(doc) {
               ...
               {
                  $( "#"+doc.aadhaar).prop( "checked", true );
               }
               if(doc.attendance == "Absent")
               {
                  $( "#"+doc.aadhaar).prop( "checked", false );
               }
      ...
      localADB.find({
        ...
        readAttend = document.getElementById(doc.aadhaar);
            if(readAttend.checked){
              localADB.get(doc._id).then(function(doc) {
                doc.attendance = "Present"
                console.log("Prdsds")
                return localADB.put(doc);
              }).then(function () {
                return localADB.get(doc._id);
              }).then(function (doc) {
                console.log(doc);
              });
            }
